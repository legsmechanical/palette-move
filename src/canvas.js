/* GENERATED by schwung-canvaskit v17 — DO NOT EDIT THIS FILE.
...
/* 7 rows: 1px pad + 5px glyphs + 1px pad — no dead rows under the text. */
const HDR_H = 7;
...
function drawChrome(ctx, headerLabel, s, swap) {
  const t = fitText(ctx, headerLabel, ctx.width - 4);
  if (swap) {
    ctx.print(Math.max(2, Math.round((ctx.width - ctx.measureText(t)) / 2)), 1, t, 1);
  } else {
    // filled bar rows 0-6 (text 1-5), then the indicator bar on row 8
    // (+9 active). The bar doubles as a page indicator (movy's drawBankBar):
    // one segment per bank split by 1px dividers, the ACTIVE bank's segment
    // 2px tall.
...
/* Layout: 7px header + blank row + the indicator bar, then two 16px widget
 * rows each with its 7px label strip DIRECTLY beneath (the knob circle is
 * 15px in the 16px cell, so knobs keep a 1px visual gap; squares sit flush):
 * hdr 0-6 (text 1-5, blank 6-7) | bar 8 (+9 active) | gap 10-11 |
...
function drawStepEditor(ctx, bank, s) {
  ...
  if (s.lastKnob >= 0 && page * 8 + s.lastKnob < count) {
    ...
    ctx.print(ctx.width - 2 - ctx.measureText(rd), 1, rd, 0);
  }
