/* GENERATED by schwung-canvaskit v16 — DO NOT EDIT THIS FILE.
...
/* Header bar: normally white-on-black with a rule along its bottom row
 * (bank label, left-aligned; icons live on the picker rows). While a knob
 * is touched/turned (`swap`), it INVERTS to a filled bar with the param
 * name in black, centered — the state change is the touch feedback. */
function drawChrome(ctx, headerLabel, s, swap) {
  const t = fitText(ctx, headerLabel, ctx.width - 4);
  if (swap) {
    ctx.fillRect(0, 0, ctx.width, HDR_H, 1);
    ctx.print(Math.max(2, Math.round((ctx.width - ctx.measureText(t)) / 2)), 1, t, 0);
  } else {
    // text rows 1-5, blank rows 6-7, indicator bar on row 8 (+9 active).
...
function drawStepEditor(ctx, bank, s) {
  ...
  if (s.lastKnob >= 0 && page * 8 + s.lastKnob < count) {
    ...
    ctx.print(ctx.width - 2 - ctx.measureText(rd), 1, rd, 1);
  }
