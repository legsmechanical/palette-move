/* GENERATED by schwung-canvaskit v5 — DO NOT EDIT THIS FILE.
...
/* Inverted header bar: bank label only (icons live on the picker rows).
 * `centered` = the touched-knob param-name swap. */
function drawChrome(ctx, headerLabel, s, centered) {
  ctx.fillRect(0, 0, ctx.width, HDR_H, 1);
  const t = fitText(ctx, headerLabel, ctx.width - 4);
  const x = centered ? Math.max(2, Math.round((ctx.width - ctx.measureText(t)) / 2)) : 2;
  ctx.print(x, 1, t, 0);
}
