/* GENERATED by schwung-canvaskit v12 — DO NOT EDIT THIS FILE.
...
function drawChrome(ctx, headerLabel, s, swap) {
  ...
  if (swap) {
    ...
    // text rows 1-5, blank row 6, rule on row 7 (just below HDR_H).
    // The rule doubles as a page indicator (movy's drawBankBar): one segment
    // per bank split by 1px dividers, the ACTIVE bank's segment 2px tall.
    ctx.print(2, 1, t, 1);
    const n = BANKS.length;
    if (n <= 1) {
      ctx.fillRect(0, HDR_H, ctx.width, 1, 1);
    } else {
      const segW = Math.floor((ctx.width - (n - 1)) / n);
      for (let b = 0; b < n; b++) {
        const sx = b * (segW + 1);
        const sw = b === n - 1 ? ctx.width - sx : segW;
        ctx.fillRect(sx, HDR_H, sw, b === s.bank ? 2 : 1, 1);
      }
    }
  }
}
...
/* Layout: 7px header (+ its rule on row 7), then two 17px widget rows each
 * with a 7px label strip beneath, filling the full 64 rows exactly:
 * hdr 0-6 | rule 7 | gap 8-10 | w0 11-27 | gap 28 | lbl0 29-35 |
 * gap 36-38 | w1 39-55 | gap 56 | lbl1 57-63. */
const ROW0_Y = 11, LBL0_Y = 29, ROW1_Y = 39, LBL1_Y = 57;
const CELL_W = 32, KW = 20, KH = 17, LBL_H = 7;
...
function drawHBar(ctx, kx, ky, norm) {
  ctx.fillRect(kx + 1, ky + 5, 18, 1, 1);
  ctx.fillRect(kx + 1, ky + 11, 18, 1, 1);
  ctx.fillRect(kx + 1, ky + 5, 1, 7, 1);
  ctx.fillRect(kx + 18, ky + 5, 1, 7, 1);
  const fillW = Math.round(norm * 16);
  if (fillW > 0) ctx.fillRect(kx + 2, ky + 6, fillW, 5, 1);
}

/* Vertical bar filling bottom->up — mix/level feel (env Vel cells). */
function drawVBar(ctx, kx, ky, norm) {
  ctx.fillRect(kx + 6, ky + 1, 8, 1, 1);
  ctx.fillRect(kx + 6, ky + 15, 8, 1, 1);
  ctx.fillRect(kx + 6, ky + 1, 1, 15, 1);
  ctx.fillRect(kx + 13, ky + 1, 1, 15, 1);
  const fillH = Math.round(norm * 13);
  if (fillH > 0) ctx.fillRect(kx + 7, ky + 2 + (13 - fillH), 6, fillH, 1);
}
...
function drawStepEditor(ctx, bank, s) {
  ...
  const dY = HDR_H + 4, dBot = ctx.height - 2, dH = dBot - dY; // frame top clears the header rule
