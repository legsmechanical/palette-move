/* GENERATED by schwung-canvaskit v3 — DO NOT EDIT THIS FILE.
...
function readState(ctx) {
  ...
  if (!s.init) {
    ...
    s.jogTouch = false;    // finger resting on the jog (note 9) -> picker
...
/* Section-picker overlay — shown while SHIFT is held OR a finger rests on
 * the jog (capacitive note 9 — free position feedback). One row per
...
function drawBankPicker(ctx, s) {
  if (!shiftHeld(s) && !s.jogTouch) return;
...
function drawBankView(ctx, bank, cells, s) {
  drawChrome(ctx, headerFor(ctx, bank, s), s);
  if (bank.env) {
...
const bank_editor = {
  ...
  _test: Object.assign({
    ...
    cellsFor, headerFor, getRaw, subPageCount, stepCells, stepVals, shapeSample, hudCard
  }, CONFIG.testExports || {})
};
