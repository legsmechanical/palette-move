/* GENERATED by schwung-canvaskit v11 — DO NOT EDIT THIS FILE.
...
function drawChrome(ctx, headerLabel, s, swap) {
  ...
  if (swap) {
    ...
    // The rule doubles as a page indicator: one segment per bank, split by
    // 1px dividers; the ACTIVE bank's segment is left blank (a break).
    ctx.print(2, 1, t, 1);
    const n = BANKS.length;
    for (let i = 0; i < n; i++) {
      if (i === s.bank) continue;
      const x0 = Math.round(i * ctx.width / n);
      const x1 = Math.round((i + 1) * ctx.width / n) - (i < n - 1 ? 1 : 0);
      ctx.fillRect(x0, HDR_H, x1 - x0, 1, 1);
    }
  }
}
