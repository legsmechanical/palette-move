/* GENERATED by schwung-canvaskit v15 — DO NOT EDIT THIS FILE.
...
/* ---- schwung host font (5x7, auto-trimmed proportional; extracted from
 * the host's scripts/generate_font.py FONT dict — the same glyphs the host
 * bakes into font.png, so the HEADER matches the rest of the device UI).
 * Embedded (not calling the host's print binding) so off-device previews
 * render pixel-identically. Glyph format: [trimmedWidth, ...7 rowBits],
 * bit0 = leftmost; advance = width + 1 (the host's charSpacing). [5] alone
 * = blank glyph (space), full cell width. ASCII 32-126. ---- */
var SF_G = [
  [5], [1,1,1,1,1,1,0,1], [3,5,5,5,0,0,0,0], [5,10,10,31,10,31,10,10], [5,4,30,5,14,20,15,4], [5,19,11,8,4,2,26,25], [5,6,9,5,2,21,9,22], [2,3,2,1,0,0,0,0],
  [3,4,2,1,1,1,2,4], [3,1,2,4,4,4,2,1], [5,0,10,4,31,4,10,0], [5,0,4,4,31,4,4,0], [2,0,0,0,0,3,2,1], [5,0,0,0,31,0,0,0], [2,0,0,0,0,0,3,3], [5,16,8,8,4,2,2,1],
  [5,14,17,25,21,19,17,14], [3,2,3,2,2,2,2,7], [5,14,17,16,8,4,2,31], [5,31,8,4,8,16,17,14], [5,8,12,10,9,31,8,8], [5,31,1,15,16,16,17,14], [5,12,2,1,15,17,17,14], [5,31,16,8,4,2,2,2],
  [5,14,17,17,14,17,17,14], [5,14,17,17,30,16,8,6], [2,0,3,3,0,3,3,0], [2,0,3,3,0,3,2,1], [4,8,4,2,1,2,4,8], [5,0,0,31,0,31,0,0], [4,1,2,4,8,4,2,1], [5,14,17,16,8,4,0,4],
  [5,14,17,16,22,21,21,14], [5,14,17,17,17,31,17,17], [5,15,17,17,15,17,17,15], [5,14,17,1,1,1,17,14], [5,7,9,17,17,17,9,7], [5,31,1,1,15,1,1,31], [5,31,1,1,7,1,1,1], [5,14,17,1,1,25,17,14],
  [5,17,17,17,31,17,17,17], [3,7,2,2,2,2,2,7], [5,28,8,8,8,8,9,6], [5,17,9,5,3,5,9,17], [5,1,1,1,1,1,1,31], [5,17,27,21,17,17,17,17], [5,17,17,19,21,25,17,17], [5,14,17,17,17,17,17,14],
  [5,15,17,17,15,1,1,1], [5,14,17,17,17,21,9,22], [5,15,17,17,15,5,9,17], [5,30,1,1,14,16,16,15], [5,31,4,4,4,4,4,4], [5,17,17,17,17,17,17,14], [5,17,17,17,17,17,10,4], [5,17,17,17,21,21,27,17],
  [5,17,17,10,4,10,17,17], [5,17,17,10,4,4,4,4], [5,31,16,8,4,2,1,31], [3,7,1,1,1,1,1,7], [5,1,1,2,4,8,16,16], [3,7,4,4,4,4,4,7], [5,4,10,17,0,0,0,0], [5,0,0,0,0,0,0,31],
  [3,1,2,4,0,0,0,0], [5,0,0,14,16,30,17,30], [5,1,1,13,19,17,17,15], [4,0,0,14,1,1,1,14], [5,16,16,22,25,17,17,30], [5,0,0,14,17,31,1,14], [5,12,18,2,7,2,2,2], [5,0,0,30,17,30,16,12],
  [5,1,1,13,19,17,17,17], [3,2,0,3,2,2,2,7], [4,8,0,12,8,8,9,6], [4,1,1,9,5,3,5,9], [3,3,2,2,2,2,2,7], [5,0,0,11,21,21,17,17], [5,0,0,13,19,17,17,17], [5,0,0,14,17,17,17,14],
  [5,0,0,15,17,15,1,1], [5,0,0,22,25,30,16,16], [5,0,0,13,19,1,1,1], [5,0,0,14,1,14,16,15], [5,2,2,7,2,2,18,12], [5,0,0,17,17,17,25,22], [5,0,0,17,17,17,10,4], [5,0,0,17,17,21,21,10],
  [5,0,0,17,10,4,10,17], [5,0,0,17,17,30,16,14], [5,0,0,31,8,4,2,31], [3,4,2,2,1,2,2,4], [1,1,1,1,1,1,1,1], [3,1,2,2,4,2,2,1], [5,0,0,2,21,8,0,0]
];

function sfGlyph(cp) { return (cp < 0x20 || cp > 0x7E) ? null : SF_G[cp - 0x20]; }

/* Width of `text` in the host font (trimmed widths + 1px spacing, no
 * trailing gap). */
function sfWidth(text) {
  var s = String(text), w = 0;
  for (var i = 0; i < s.length; i++) {
    var g = sfGlyph(s.charCodeAt(i));
    w += (g ? g[0] : 5) + 1;
  }
  return Math.max(0, w - 1);
}

function sfPrint(ctx, x, y, text, color) {
  ...
  for (var i = 0; i < s.length; i++) {
    ...
    for (var r = 0; r < 7; r++) {
      var bits = g[1 + r] || 0;
      for (var c = 0; c < g[0]; c++) if (bits & (1 << c)) ctx.setPixel(cx + c, oy + r, v);
    }
    cx += g[0] + 1;
  }
}
...
/* 9 rows: 1px pad + 7px host-font glyphs + 1px pad. */
const HDR_H = 9;
...
/* Header bar, rendered in the HOST's 5x7 font (sfPrint) so it matches the
 * rest of the device UI. Normally white-on-black with the page-indicator
 * bar below (bank label, left-aligned; icons live on the picker rows).
 ...
 * feedback. */
function fitSf(text, maxW) {
  let t = String(text);
  while (t.length > 0 && sfWidth(t) > maxW) t = t.slice(0, -1);
  return t;
}

function drawChrome(ctx, headerLabel, s, swap) {
  const t = fitSf(headerLabel, ctx.width - 4);
  if (swap) {
    ctx.fillRect(0, 0, ctx.width, HDR_H, 1);
    sfPrint(ctx, Math.max(2, Math.round((ctx.width - sfWidth(t)) / 2)), 1, t, 0);
  } else {
    // text rows 1-7, blank rows 8-9, indicator bar on row 10 (+11 active).
    // The bar doubles as a page indicator (movy's drawBankBar): one segment
    // per bank split by 1px dividers, the ACTIVE bank's segment 2px tall.
    sfPrint(ctx, 2, 1, t, 1);
    const barY = HDR_H + 1;
...
/* Layout: 9px header (host 5x7 font) + blank rows + the indicator bar,
 ...
 * hdr 0-8 (text 1-7, blank 8-9) | bar 10 (+11 active) | gap 12 |
 * w0 13-28 | lbl0 29-35 | gap 36-39 | w1 40-55 | lbl1 56-62 | blank 63. */
const ROW0_Y = 13, LBL0_Y = 29, ROW1_Y = 40, LBL1_Y = 56;
